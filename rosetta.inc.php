<?php
	/* 
		Rosetta Translation Framework v2 (with GUI)
		...
		XML though. You are free to store the XML in a database, raw file or any other means.
	*/
	...
	class rosetta
	{
		...
		/**
			Initialize the constructor given a string and a method.
			@str String Either the filepath to an XML doc or an XML string literal.
			@method Int One of rosetta::LOAD_STRING or rosetta::LOAD_URL which specifies what @str is.
		*/
		public function __construct($str, $method=rosetta::LOAD_STRING)
		{
			if($method == rosetta::LOAD_URL) {
				$file = fopen($str, 'r');
				$text = fread($file, filesize($str));
				fclose($file);
...
<?php
		...
		*/
		public function getString($id, $args = array())
		{
			return $this->currentLang->getString($id, $args);
		}
	...
	class language
	{
		...
		public function getString($id, $args = array())
		{
			if(array_key_exists($id, $this->strings)) {
				$tmp = $this->strings[$id]->string;
				
				//replace the [key] instances in the XML string with their supplied values
				$argKeys = array_keys($args);
				foreach($argKeys as $key) {
					$tmp = str_replace("[" . $key . "]", $args[$key], $tmp);
				}

				//make the text field editable for admins
				if (session_status() == PHP_SESSION_ACTIVE) {
					if(array_key_exists('isLoggedIn', $_SESSION)) {
					    if($_SESSION['isLoggedIn'] == "true") {
					    	if($this->strings[$id]->type == "text") {
					    		return "<span id = '$id' class = 'edit'>" . $tmp . "</span>";
					    	}
					    }
					}
				}
